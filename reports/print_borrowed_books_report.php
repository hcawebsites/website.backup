...
class myPDF extends FPDF{
    ...
    function title(){ 
        ...
        $this->Cell(1);
        $this->Cell(0,0,'Borrowed Books Reports',0,0,'C');
        $this->Ln(7);
        $this->SetFont('Times','',12);
        $this->Cell(1);
    ...
    function content(){
        include '../database/connection.php';
        $this->SetFont('Times','B',12);
        $this->SetWidths(array(10, 45, 40, 35, 40, 35, 35, 35));
        for($i=0;$i<1;$i++):
        $this->Row(array('#', 'ISBN', 'Title', 'Author', 'Borrowers ID', 'Name', 'Contact', 'Transaction Date'));
        endfor;

        $sql = "SELECT * FROM borrow_books Where Status = '1'";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $this->SetWidths(array(10, 45, 40, 35, 40, 35, 35, 35));
            for($i=0;$i<1;$i++):
            $this->Row(array(''.$row['ID'].'', ''.$row['ISBN'].'', ''.$row['Title'].'', ''.$row['Author'].'', ''.$row['Borrowers_ID'].'', ''.$row['Fullname'].'', ''.$row['Contact'].'', ''.$newdate.''));
            endfor;
        }
