...
class myPDF extends FPDF{
    ...
    function content(){ 
        ...
        $this->SetFont('Times','B',12);
        $this->Cell(1);
        $this->Cell(0,0,'Medicine Reports',0,0,'C');
        $this->Ln(5);
        ...
        $this->SetFont('Times','BI',11);
            $this->SetWidths(array(15, 60, 60, 60, 45, 35, 35, 35));
            for($i=0;$i<1;$i++):
            $this->Row(array('#','Name', 'Milligrams', 'Type', 'Total', 'Date Created', 'Expiration Date'));
        endfor;

         		$get = mysqli_query($con, "SELECT * from medicine order by ID ASC");
	    		while ($row = mysqli_fetch_assoc($get)) {
                $date_created = date('F j, Y', strtotime($row['Date_Created']));
                $expiration = date('F j, Y', strtotime($row['Date_Expiration']));
                	$this->SetFont('Times','I',11);
	                $this->SetWidths(array(15, 60, 60, 60, 45, 35, 35, 35));
	                for($i=0;$i<1;$i++):
	                $this->Row(array(''.$count++.'', ''.$row['Name'].'', ''.$row['Mg'].'', ''.$row['Type'].'', ''.$row['Total'].'', ''.$date_created.'', ''.$expiration.''));
	                endfor;
         }
...
if (isset($_POST['excel'])) {
    ...
    $fileName = "medicine_reports_" . date('Y-m-d') . ".xls";

    $fields = array('#','Name', 'Milligrams', 'Type', 'Total', 'Date Created', 'Expiration Date');
    ...
    if (mysqli_num_rows($get) > 0) {
        while ($row = mysqli_fetch_assoc($get)) {
        	...
        	$data = array(''.$count++.'', ''.$row['Name'].'', ''.$row['Mg'].'', ''.$row['Type'].'', ''.$row['Total'].'', ''.$date_created.'', ''.$expiration.'');
