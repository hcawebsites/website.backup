...
$location = "../S-QRCODE/".$staff_id.".png";

$tqrcode = $staff_id.".png";
...
if (in_array($img_ex_lc, $allowed_exs)) {
    ...
    $check_email = mysqli_query($con, "SELECT * FROM staff_tb Where Email = '$email'");
    if (mysqli_num_rows($check_email)) {
        echo 'Email Already Existed';
    }else{
        $insert_query = mysqli_query($con, "INSERT INTO staff_tb (Emp_ID, Salutation, Lastname, Firstname, Middlename, Suffix, DOB, Age, Gender, Status, Address, Nationality, Contact, Email, Cashier, Picture, QR_Code, Date)
    VALUES ('$staff_id', '$salutation', '$lastname', '$firstname', '$middlename', '$suffix', '$birthdate', '$age', '$gender', '$status', '$address', '$nationality', '$phone', '$email', '$position', '$new_img_name', '$tqrcode', '$reg_date')");
    ...
    if ($insert_query) {
        $subject = "Account Confirmation";
            ...
            if(mail($email, $subject, $htmlContent, $headers)){
                mysqli_query($con, "INSERT INTO user (Username, Lastname, Firstname, Middlename, Contact, Email, Password, Access, AStatus) VALUES ('$staff_id', '$lastname', '$firstname', '$middlename', '$phone', '$email', '$password', '$access', '1')");
                QRcode::png($staff_id, $location , 150, 150);
                echo 'success';
            }
    }
